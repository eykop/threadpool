...
int main(int argc, char* argv[])
{
    ...
    auto commandArgsParsingResult = userinpututils::parseCommandLineArguments(parser);
    ...
    tp.setNotifyOnAllFinished([]() {
        QCoreApplication::exit();
    });

    // we manage the threads/tasks in a separate thread so that we can allow the application to "work" on other things.
    ...
    //make sure we allow the manage thread to finish before be terminate the application.
